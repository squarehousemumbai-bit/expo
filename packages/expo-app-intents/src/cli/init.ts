...
/**
 * Returns whether any watched entry reaches Swift files outside `directory`.
 *
 * Only such an entry can compile a second `AppIntentsSetup.swift` into the app target. An entry
 * that is `directory` itself, or something inside it, covers nothing but the directory this run
 * scaffolds, so no other intents directory can be in the build to collide with it.
 */
function watchesAnythingOutside(
  projectRoot: string,
  watchedDirectories: string[],
  directory: string
): boolean {
  return watchedDirectories.some(
    (watchedDirectory) => !isSameOrInside(projectRoot, directory, watchedDirectory)
  );
}
 ...
 */
function assertDirectoryIsNotAlreadyConfigured(
  projectRoot: string,
  ...
  if (!pluginEntry) {
    ...
    // not mean the project has none. That only matters while a directory outside this one is
    // watched: a plugin entry init cannot see needs a second watched directory to point at before
    // two copies of AppIntentsSetup.swift can reach the app target. A project that watches nothing
    // yet, or watches only the directory this run scaffolds, has nothing to collide with.
    if (
      config.dynamicConfigPath &&
      watchesAnythingOutside(projectRoot, getWatchedDirectories(config), directory)
    ) {
      console.warn(
        `Could not check whether expo-app-intents is already configured for another directory: the ` +
          `plugins list is computed in JavaScript in ` +
          `${path.relative(projectRoot, config.dynamicConfigPath)}, and init can only read the ` +
          `static config. If the plugin already points at a directory other than '${directory}', ` +
          `keep one of them: autolinking compiles every watched directory into the app target, and ` +
          `two directories that both hold an AppIntentsSetup.swift make the iOS build fail with ` +
...
export async function runInit(options: InitOptions): Promise<void> {
  ...
  assertDirectoryIsNotAlreadyConfigured(projectRoot, config, directory);

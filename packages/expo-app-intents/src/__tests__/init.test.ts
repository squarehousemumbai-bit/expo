...
`);
  ...
  // A dynamic config can point the plugin at a directory init cannot see, and a watched directory
  // outside the requested one is already in the app target, so it could hold a second scaffold.
  it('warns that the plugin check cannot run against a dynamic config that watches other directories', async () => {
    writeProject(null);
    write({
      'project/app.config.js': `module.exports = { expo: { name: 'my-app', slug: 'my-app', experiments: { inlineModules: { watchedDirectories: ['.'] } } } };`,
    });

    await runInit({ projectRoot, directory: 'app-intents', examples: ['minimal'], templatesDir });

    expect(messages(warn)).toContain('plugins list is computed in JavaScript');
  });
